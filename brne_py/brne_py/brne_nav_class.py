...
class BrneNavClass(Node):
    def __init__(self):
        super().__init__('brne_nav_class')
        ...
        self.brne = BRNE(kernel_a1=self.kernel_a1,
                               kernel_a2=self.kernel_a2,
                               cost_a1=self.cost_a1,
                               cost_a2=self.cost_a2,
                               cost_a3=self.cost_a3,
                               dt=self.dt,
                               plan_steps=self.plan_steps,
                               n_samples=self.num_samples,
                               max_ang_vel=self.max_ang_vel,
                               max_lin_vel=self.max_lin_vel,
                               nominal_vel=self.nominal_vel,
                               ymin=self.corridor_y_min,
                               ymax=self.corridor_y_max
                               )
    ...
    def brne_cb(self):
        ...
        self.brne.update_peds(ped_info_list)
        ...
        if num_agents > 1:
            ...
            x_pts = self.brne.mvn_sample_normal()
            y_pts = self.brne.mvn_sample_normal()
            ...
            ulist_essemble = self.brne.get_ulist_essemble(nominal_cmds)
            traj_essemble = self.brne.traj_sim_essemble(
            ...
            weights = self.brne.brne_nav(
                xtraj_samples=xtraj_samples, 
